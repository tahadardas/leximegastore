...
class Lexi_Routes_Orders
{
    ...
    public static function register(): void
    {
        ...
        register_rest_route($ns, '/my-orders/(?P<id>\d+)/confirm-received', array(
            'methods' => WP_REST_Server::CREATABLE,
            'callback' => array(__CLASS__, 'confirm_order_received'),
            'permission_callback' => array('Lexi_Security', 'customer_access'),
            'args' => array(
                'id' => array('required' => true, 'sanitize_callback' => 'absint'),
            ),
        ...
        register_rest_route($ns, '/my-orders/(?P<id>\d+)/refuse', array(
            'methods' => WP_REST_Server::CREATABLE,
            'callback' => array(__CLASS__, 'refuse_order'),
            'permission_callback' => array('Lexi_Security', 'customer_access'),
            'args' => array(
                'id' => array('required' => true, 'sanitize_callback' => 'absint'),
...
<body>
    ...
    public static function format_order(WC_Order $order, bool $include_private_location = false): array
    {
        ...
        foreach ($order->get_items() as $item) {
            ...
            $items[] = array(
                ...
                'variation_label' => Lexi_Order_Flow::get_item_variation_label($item),
    ...
    /**
     * POST /my-orders/{id}/confirm-received
     */
    public static function confirm_order_received(WP_REST_Request $request): WP_REST_Response
    {
        $user_id = get_current_user_id();
        $order_id = absint($request->get_param('id'));

        $order = wc_get_order($order_id);
        if (!$order || !self::user_owns_order($order, $user_id)) {
            return Lexi_Security::error('order_not_found', 'الطلب غير موجود.', 404);
        }

        $status = self::normalize_public_status((string) $order->get_status());
        if ($status === 'completed') {
            return Lexi_Security::success(array('message' => 'الطلب مكتمل بالفعل.'));
        }

        $order->update_status('completed', 'تم تأكيد الاستلام بواسطة الزبون عبر التطبيق.');
        $order->add_order_note('تأكيد الاستلام بواسطة الزبون (ID: ' . $user_id . ').');
        $order->save();
    ...
    /**
     * POST /my-orders/{id}/refuse
     */
    public static function refuse_order(WP_REST_Request $request): WP_REST_Response
    {
        $user_id = get_current_user_id();
        $order_id = absint($request->get_param('id'));
        ...
        if (empty($reason)) {
            return Lexi_Security::error('missing_reason', 'سبب الرفض مطلوب إجبارياً.', 422);
        }
        ...
        $status = self::normalize_public_status((string) $order->get_status());
        if (in_array($status, array('completed', 'cancelled', 'failed', 'refunded'), true)) {
            return Lexi_Security::error('invalid_status', 'لا يمكن رفض الطلب في حالته الحالية.', 422);
        }

        $order->update_status('cancelled', 'تم رفض الاستلام بواسطة الزبون. السبب: ' . $reason);
        $order->add_order_note('رفض الاستلام بواسطة الزبون (ID: ' . $user_id . '). السبب: ' . $reason);
        $order->save();

        // Notify admins about refusal
        if (class_exists('Lexi_Push')) {
            Lexi_Push::send_push_for_target(array(
                'target' => 'all_admins',
                'audience' => 'admin',
                'title_ar' => 'رفض استلام طلب',
                'body_ar' => sprintf('قام الزبون برفض الطلب #%s. السبب: %s', $order->get_order_number(), $reason),
                'type' => 'order_refused_by_customer',
                'order_id' => $order_id,
            ));
        }

        return Lexi_Security::success(array(
            'message' => 'تم تسجيل رفض الاستلام بنجاح.',
            'status' => 'cancelled'
        ));
    }
}
