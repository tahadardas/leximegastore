...
class Lexi_Order_Flow
{
    ...
    /**
     * Format the item name to include its variation attributes.
     * Often used to ensure color/size selected by the customer is visible
     * in the app or emails.
     */
    public static function format_order_item_name(WC_Order_Item $item): string
    {
        $name = (string) $item->get_name();
        $variation_label = self::get_item_variation_label($item);
    ...
    /**
     * Get the variation label (attributes) for an order item.
     * e.g. "Color: Red, Size: Large"
     */
    public static function get_item_variation_label(WC_Order_Item $item): string
    {
        ...
        foreach ($formatted_meta as $meta) {
            // Skip hidden internal meta
            if (is_string($meta->display_key) && strpos($meta->display_key, '_') === 0) {
                continue;
            }
            $meta_strings[] = wp_strip_all_tags(trim((string) $meta->display_key)) . ': ' . wp_strip_all_tags(trim((string) $meta->display_value));
        }

        return implode(', ', $meta_strings);
    }
}
