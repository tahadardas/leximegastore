...
class Lexi_Emails
{
    ...
    public static function send_internal_order_email(WC_Order $order): bool
    {
        ...
        self::log(
            'wp_mail result for order #' . $order_number . ': ' . ($sent ? 'sent' : 'failed')
    ...
    public static function send_test_email(array $context = array()): array
    {
        ...
        $body = sprintf(
            ...
            !empty($context)
                ? '<p style="margin:0 0 6px;"><strong>سياق:</strong> ' . esc_html(wp_json_encode($context)) . '</p>'
                : ''
        );
        ...
        self::log('Test email result: ' . ($sent ? 'sent' : 'failed') . ' | recipients=' . json_encode($recipients));
