...
class Lexi_Merch
{
    ...
    {
        $woo_order = get_term_meta($term_id, 'order', true);
        if ('' !== (string) $woo_order) {
            return (int) $woo_order;
        }

        $lexi_order = get_term_meta($term_id, self::CATEGORY_SORT_META, true);
        return '' !== (string) $lexi_order ? (int) $lexi_order : 0;
    }

    public static function set_category_sort_order(int $term_id, int $sort_order, bool $invalidate = true): void
    {
        $normalized = max(0, $sort_order);
        update_term_meta($term_id, self::CATEGORY_SORT_META, $normalized);
        update_term_meta($term_id, 'order', $normalized);
        clean_term_cache(array($term_id), 'product_cat');
        if ($invalidate) {
            self::invalidate_category_ordering_cache();
        }
    }
    ...
    public static function get_sorted_categories(bool $hide_empty = true): array
    {
        ...
        foreach ($terms as $index => $term) {
            $positions[(int) $term->term_id] = $index;
        }

        usort($terms, function ($a, $b) use ($positions) {
            $a_id = (int) $a->term_id;
            $b_id = (int) $b->term_id;
            $a_order = self::get_category_sort_order($a_id);
            $b_order = self::get_category_sort_order($b_id);

            if ($a_order !== $b_order && ($a_order > 0 || $b_order > 0)) {
                return $a_order <=> $b_order;
            }
            return ($positions[$a_id] ?? 0) <=> ($positions[$b_id] ?? 0);
        });
    ...
    /**
     * Flatten product categories using the same parent/child traversal expected in WordPress admin.
     *
     * @return array<int, array{term: WP_Term, depth: int}>
     */
    public static function get_admin_ordered_category_rows(bool $hide_empty = true): array
    {
        // By using 'name', we encourage third-party drag-and-drop plugins 
        // to natively inject their specific sort order into the WP query.
        $terms = get_terms(array(
            'taxonomy' => 'product_cat',
            'hide_empty' => $hide_empty,
            'orderby' => 'name',
        ...
        foreach ($children_by_parent as &$siblings) {
            usort($siblings, function ($a, $b) use ($positions) {
                $a_id = (int) $a->term_id;
                $b_id = (int) $b->term_id;
                $a_order = self::get_category_sort_order($a_id);
                $b_order = self::get_category_sort_order($b_id);

                // If explicit custom numeric order is used, respect it.
                // Otherwise, the natural order returned by get_terms (which respects plugins) is used via positions metadata.
                if ($a_order !== $b_order && ($a_order > 0 || $b_order > 0)) {
                    return $a_order <=> $b_order;
                }
        ...
        $append_children = static function (
            int $parent_id,
            int $depth
        ) use (&$append_children, &$children_by_parent, &$rows, &$visited): void {
            $siblings = $children_by_parent[$parent_id] ?? array();
            foreach ($siblings as $term) {
                ...
                $append_children($term_id, $depth + 1);
            }
        };
     ...
     * @return array<int, int>
     */
    public static function get_category_product_ids_by_menu_order(int $term_id, string $search = ''): array
    {
        if ($term_id <= 0) {
            return array();
        }
    ...
    {
        $all_ids = self::get_category_product_ids_by_menu_order($term_id, $search);
        if (empty($all_ids)) {
            return array();
        }
    ...
    /**
     * Lightweight product formatter for listing endpoints.
     *
     * Keeps response keys backward-compatible while avoiding expensive
     * variation/attribute/gallery hydration used by detail endpoints.
     *
     * @param array<int, array{pinned: bool, sort_order: int}>|null $order_map
     * @return array<string, mixed>
     */
    public static function format_product_for_list_api(
        WC_Product $product,
        ...
        if ($thumb_id > 0) {
            $primary_image = self::build_image_size_payload($thumb_id);
        }

        $featured_image = isset($primary_image['medium']) && is_string($primary_image['medium']) && '' !== trim($primary_image['medium'])
            ? $primary_image['medium']
            : (
                isset($primary_image['thumb']) && is_string($primary_image['thumb']) && '' !== trim($primary_image['thumb'])
                    ? $primary_image['thumb']
                    : (
                        isset($primary_image['large']) && is_string($primary_image['large']) && '' !== trim($primary_image['large'])
                            ? $primary_image['large']
                            : null
                    )
            );
        ...
        $images = array();
        $card_images = array();
        if (is_string($featured_image) && '' !== trim($featured_image)) {
            $images[] = $featured_image;
            $card_images[] = $featured_image;
        }

        $gallery_ids = array_slice(array_values(array_map('intval', $product->get_gallery_image_ids())), 0, 2);
        foreach ($gallery_ids as $gallery_id) {
            if ($gallery_id <= 0) {
                continue;
            }

            $sizes = self::build_image_size_payload($gallery_id);
            $detail = isset($sizes['large']) && is_string($sizes['large']) && '' !== trim($sizes['large'])
                ? $sizes['large']
                : (
                    isset($sizes['medium']) && is_string($sizes['medium']) && '' !== trim($sizes['medium'])
                        ? $sizes['medium']
                        : (
                            isset($sizes['thumb']) && is_string($sizes['thumb']) && '' !== trim($sizes['thumb'])
                                ? $sizes['thumb']
                                : null
                        )
                );
            $card = isset($sizes['medium']) && is_string($sizes['medium']) && '' !== trim($sizes['medium'])
                ? $sizes['medium']
                : (
                    isset($sizes['thumb']) && is_string($sizes['thumb']) && '' !== trim($sizes['thumb'])
                        ? $sizes['thumb']
        ...
        $images = array_values(array_unique(array_values(array_filter($images, function ($url) {
            return is_string($url) && '' !== trim($url);
        }))));
        $card_images = array_values(array_unique(array_values(array_filter($card_images, function ($url) {
            return is_string($url) && '' !== trim($url);
        ...
        if (function_exists('mb_substr') && mb_strlen($description) > 320) {
            $description = (string) mb_substr($description, 0, 320);
    ...
    private static function extract_variations_for_api(WC_Product $product): array
    {
        ...
        foreach ($children as $variation_id) {
            ...
            foreach ($raw_attributes as $attr_key => $attr_value) {
                ...
                // If this is a taxonomy attribute (starts with pa_), we need to get the term name.
                ...
                // Get the human-readable attribute label (e.g., "pa_color" -> "Color")
                $attr_label = wc_attribute_label($attr_key, $variation);
    ...
    public static function resolve_products_for_section(array $section, int $limit = 12, bool $exclude_unpriced = true): array
    {
            ...
            if (!empty($manual_items)) {
                ...
                if (!empty($ids)) {
                    ...
                    foreach ($ids as $id) {
                        ...
                        $result[] = self::format_product_for_list_api($products[$id]);
                    }
                    return $result;
                }
            }
        ...
        if ('category' === $type) {
            ...
            foreach ($ids as $id) {
                ...
                $result[] = self::format_product_for_list_api($products[$id], $term_id, $order_map);
            }
            return $result;
        }
        ...
        foreach ($query->posts as $post_id) {
            ...
            $result[] = self::format_product_for_list_api($product);
        }
    ...
    {
        $prefixes = array(
            '_transient_lexi_products_list_v4_',
            '_transient_timeout_lexi_products_list_v4_',
            '_transient_lexi_products_list_v3_',
            '_transient_timeout_lexi_products_list_v3_',
            '_transient_lexi_home_sections_v2_',
            '_transient_timeout_lexi_home_sections_v2_',
            '_transient_lexi_categories_list_v2_',
            '_transient_timeout_lexi_categories_list_v2_',
        );

        self::delete_transients_with_prefixes($prefixes);
    }

    /**
     * Flush public shipping transients so checkout picks city/rate updates
     * immediately instead of waiting for TTL expiry.
     */
    public static function invalidate_shipping_cache(): void
    {
        delete_transient('lexi_shipping_cities_v1');
        self::delete_transients_with_prefixes(array(
            '_transient_lexi_shipping_rate_v1_',
            '_transient_timeout_lexi_shipping_rate_v1_',
        ));
    }

    /**
     * Clear the narrow caches affected by product category ordering changes.
     */
    public static function invalidate_category_ordering_cache(): void
    {
        if (function_exists('clean_taxonomy_cache')) {
            clean_taxonomy_cache('product_cat');
        }

        delete_transient('lexi_categories_page_slugs_v1');
        self::invalidate_products_list_cache();
    }

    /**
     * @param array<int, string> $prefixes
     */
    private static function delete_transients_with_prefixes(array $prefixes): void
    {
        global $wpdb;

        foreach ($prefixes as $prefix) {
            $safe_prefix = trim((string) $prefix);
            if ('' === $safe_prefix) {
                continue;
            }
