...
class Lexi_Notifications
{
    ...
    public static function insert(array $args): int|false
    {
        ...
        // Sanitize
        $data['title_ar'] = sanitize_text_field(self::normalize_text($data['title_ar']));
        $data['body_ar'] = sanitize_textarea_field(self::normalize_text($data['body_ar']));
     ...
     */
    public static function broadcast_customer(
        string $type,
        string $title_ar,
        string $body_ar,
        ?array $data_json = null
    ...
    public static function notify_customer(
        ...
        $safe_device_id = $device_id ? sanitize_text_field($device_id) : null;

        if (
            self::is_duplicate_customer_order_notification(
                $user_id,
                $safe_device_id,
                $type,
                $order_id
    ...
    private static function is_duplicate_customer_order_notification(
        ?int $user_id,
        ?string $device_id,
        string $type,
        ?int $order_id
        ...
        if (!in_array($normalized_type, $dedupe_types, true)) {
            return false;
        }
    ...
    public static function get_notifications(
        ...
        $items = array_map(function ($row) {
            return [
                ...
                'body_ar' => self::normalize_text($row['body_ar'] ?? ''),
    ...
    public static function append_timeline(WC_Order $order, string $type, string $message_ar): void
    {
        ...
        $message_ar = trim(wp_strip_all_tags(self::normalize_text($message_ar)));
        if ('' === $type || '' === $message_ar) {
            return;
        }
    ...
    public static function notify_customer_decision(WC_Order $order, string $decision, string $note_ar = ''): void
    {
        ...
        if (class_exists('Lexi_Push')) {
            $push_target = '';
            if ($user_id > 0) {
                $push_target = 'specific_user';
            } elseif ($device_id !== '') {
                $push_target = 'specific_device';
            }
            ...
            if ($push_target !== '') {
                Lexi_Push::send_push_for_target(array(
                    ...
                    'title_ar' => $title,
                    'body_ar' => $message,
                    'type' => 'order_' . $decision,
                    'open_mode' => 'in_app',
                    'deep_link' => '/orders/status?order_number=' . rawurlencode($order_number),
                    'priority' => 'high',
                    'order_id' => (int) $order->get_id(),
                    'extra_data' => array(
        ...
        if ($table_exists) {
            ...
            if (is_array($rows)) {
                foreach ($rows as $row) {
                    ...
                    $items[] = array(
                        'title' => sanitize_text_field(self::normalize_text((string) ($row['title_ar'] ?? ''))),
                        'message' => sanitize_textarea_field(self::normalize_text((string) ($row['body_ar'] ?? ''))),
    ...
    {
        $title = sanitize_text_field(self::normalize_text($title));
        $message = sanitize_textarea_field(self::normalize_text($message));
        if ('' === $title && '' === $message) {
            return;
        }
    ...
    private static function normalize_timeline_items($raw): array
    {
        ...
        foreach ($raw as $item) {
            ...
            $message = sanitize_textarea_field(self::normalize_text((string) ($item['message_ar'] ?? '')));
    ...
    private static function normalize_inbox_items($raw): array
    {
        ...
        foreach ($raw as $item) {
            ...
            $title = sanitize_text_field(self::normalize_text((string) ($item['title'] ?? '')));
            $message = sanitize_textarea_field(self::normalize_text((string) ($item['message'] ?? '')));
            $created_at = sanitize_text_field((string) ($item['created_at'] ?? ''));
