...
class Lexi_Push
{
    ...
    public static function register_token(array $payload): array
    {
        ...
        if ($device_id !== '') {
            if ($existing_id > 0) {
                $wpdb->query(
                    $wpdb->prepare(
                        "DELETE FROM {$table} WHERE device_id = %s AND id <> %d",
                        $device_id,
                        $existing_id
                    )
                );
                ...
                $wpdb->query(
                    $wpdb->prepare(
                        "DELETE FROM {$table} WHERE device_id = %s",
                        $device_id
                    )
                );
            }
        ...
        if ($existing_id > 0) {
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$table} WHERE fcm_token = %s AND id <> %d",
                    $token,
                    $existing_id
                )
            );
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM {$table} WHERE fcm_token = %s",
                    $token
                )
            );
        }
    ...
    public static function send_push_for_target(array $payload): array
    {
        ...
        if ($order_id <= 0 && isset($extra_data['order_id'])) {
            $order_id = absint((int) $extra_data['order_id']);
        }
        $status_fingerprint = sanitize_key(
            (string) ($extra_data['new_status'] ?? $extra_data['decision'] ?? '')
        );
        if ($status_fingerprint === 'approved') {
            $status_fingerprint = 'processing';
        } elseif ($status_fingerprint === 'rejected') {
            $status_fingerprint = 'failed';
        }
        ...
        if (
            self::is_duplicate_order_push_event(
                $target,
                $audience,
                $user_id,
                $device_id,
            ...
            $result['provider_status'] = 'deduplicated';
            return $result;
        }
    ...
    private static function is_duplicate_order_push_event(
        string $target,
        string $audience,
        int $user_id,
        string $device_id,
        ...
        if (!in_array($type, $dedupe_types, true)) {
            return false;
        }
        if (!in_array($target, array('specific_user', 'specific_device'), true)) {
            return false;
        }

        $recipient_key = '';
        if ($target === 'specific_user' && $user_id > 0) {
            $recipient_key = 'u:' . $user_id;
        } elseif ($target === 'specific_device' && trim($device_id) !== '') {
            $recipient_key = 'd:' . trim($device_id);
        }
        if ($recipient_key === '') {
            return false;
        }
        ...
        $transient_key = 'lexi_push_once_' . $fingerprint;

        if ((int) get_transient($transient_key) === 1) {
            return true;
        }
