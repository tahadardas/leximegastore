...
class Lexi_AI_Routes
{
    ...
    private function format_product($product, $extra = [])
    {
        ...
        $image_url = Lexi_Merch::normalize_image_url((string) $image_url);
        $prices = Lexi_Merch::resolve_product_prices($product);
        ...
        $full = Lexi_Merch::format_product_for_api($product);
        $payload = array_merge($full, [
            'id' => $product->get_id(),
            'name' => $product->get_name(),
        ...
        if (empty($payload['featured_image']) && '' !== $image_url) {
            $payload['featured_image'] = $image_url;
        }
        if (empty($payload['image']) && '' !== $image_url) {
            $payload['image'] = $image_url;
        }
        if (empty($payload['images']) && '' !== $image_url) {
            $payload['images'] = [$image_url];
        }
        if (empty($payload['card_images']) && '' !== $image_url) {
            $payload['card_images'] = [$image_url];
        }
