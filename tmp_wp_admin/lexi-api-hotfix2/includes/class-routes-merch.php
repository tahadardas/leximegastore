...
class Lexi_Routes_Merch
{
    ...
    {
        $rows = Lexi_Merch::get_admin_ordered_category_rows(false);
        $items = array();
        foreach ($rows as $index => $row) {
            ...
            $term_name = self::normalize_text((string) $term->name);
            ...
            $items[] = array(
                'id' => $term_id,
                'name' => $term_name,
                'display_name' => self::build_category_display_name($term_name, $depth),
    ...
    public static function admin_patch_categories(WP_REST_Request $request): WP_REST_Response
    {
        ...
        foreach ($items as $item) {
            ...
            Lexi_Merch::set_category_sort_order($term_id, max(0, (int) ($item['sort_order'] ?? 0)), false);
        }
    ...
    public static function admin_get_category_products(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $all_ids = Lexi_Merch::get_category_product_ids_by_menu_order($term_id, $search);
    ...
    private static function build_category_display_name(string $name, int $depth): string
    {
        if ($depth <= 0) {
            return $name;
        }
