...
class Lexi_Plugin
{
    ...
    private function __construct()
    {
        ...
        add_action('save_post_product', array($this, 'invalidate_merchandising_cache'), 20);
        add_action('woocommerce_update_product', array($this, 'invalidate_merchandising_cache'), 20);
        add_action('created_product_cat', array($this, 'invalidate_category_ordering_cache'), 20);
        add_action('edited_product_cat', array($this, 'invalidate_category_ordering_cache'), 20);
        add_action('delete_product_cat', array($this, 'invalidate_category_ordering_cache'), 20);
        add_action('added_term_meta', array($this, 'maybe_invalidate_product_cat_order_cache'), 20, 4);
        add_action('updated_term_meta', array($this, 'maybe_invalidate_product_cat_order_cache'), 20, 4);
        add_action('deleted_term_meta', array($this, 'maybe_invalidate_product_cat_order_cache'), 20, 4);
        add_action('woocommerce_after_term_ordering', array($this, 'invalidate_category_ordering_cache'), 20);
    }

    /**
     * Keep public product list caches fresh after WordPress product edits.
     */
    public function invalidate_merchandising_cache(...$args): void
    {
        if (class_exists('Lexi_Merch')) {
            Lexi_Merch::invalidate_products_list_cache();
        }
    }

    /**
     * Keep public category/product ordering fresh after taxonomy edits.
     */
    public function invalidate_category_ordering_cache(...$args): void
    {
        if (class_exists('Lexi_Merch')) {
            Lexi_Merch::invalidate_category_ordering_cache();
        }
    }

    /**
     * WooCommerce product category drag sorting writes term meta directly.
     */
    public function maybe_invalidate_product_cat_order_cache($meta_id, $term_id, $meta_key, $meta_value): void
    {
        $key = (string) $meta_key;
        if ('order' !== $key && 'lexi_sort_order' !== $key) {
            return;
        }
        ...
        $this->invalidate_category_ordering_cache();
    }
