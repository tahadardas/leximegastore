...
class Lexi_Routes_Notifications
{
    ...
    public static function register(): void
    {
        ...
        register_rest_route($ns, '/admin/notifications/clear-tokens', array(
            'methods' => WP_REST_Server::DELETABLE,
            'callback' => array(__CLASS__, 'admin_clear_tokens'),
            'permission_callback' => array(__CLASS__, 'admin_permission'),
        ));
    }
    ...
    /**
     * DELETE /admin/notifications/clear-tokens
     */
    public static function admin_clear_tokens(): WP_REST_Response
    {
        Lexi_Push::clear_all_tokens();
        return Lexi_Security::success(array(
            'message' => 'تم مسح جميع رموز الإشعارات بنجاح. سيقوم المستخدمون بإعادة التسجيل تلقائياً عند فتح التطبيق.',
        ));
    }
    ...
    public static function admin_send_notification(WP_REST_Request $request)
    {
        ...
        if (!in_array($target, array('broadcast', 'everyone', 'all_admins', 'specific_user', 'specific_device'), true)) {
        ...
        $stored_count = 0;
        if ($audience === 'admin' || $target === 'everyone') {
            if ($target === 'specific_user') {
                ...
                // If everyone, we notify admins and later we will broadcast to customers
                Lexi_Notifications::notify_admins($type, $title_ar, $body_ar, null, $notification_data);
                $stored_count = self::count_admin_users();
            }
        } 
        
        if ($audience === 'customer' || $target === 'everyone') {
            switch ($target) {
                case 'broadcast':
                case 'everyone':
                    $count = Lexi_Notifications::broadcast_customer(
                        $type,
                        $title_ar,
                        $body_ar,
                        $notification_data
