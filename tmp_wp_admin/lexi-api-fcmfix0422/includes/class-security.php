...
class Lexi_Security
{
    ...
    /**
     * REST permission callback: delivery agents only.
     */
    public static function delivery_access()
    {
        if (!is_user_logged_in()) {
            return new WP_Error(
                'rest_forbidden',
        ...
        if (current_user_can('lexi_delivery_agent')) {
            return true;
        }

        $user = wp_get_current_user();
        $roles = is_array($user->roles) ? $user->roles : array();
        return in_array('delivery_agent', $roles, true);
    }
