...
class Lexi_Routes_Delivery
{
    ...
    public static function get_orders(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $collected_orders = wc_get_orders(array(
            'limit' => -1,
            'return' => 'ids',
            'status' => 'any',
            'meta_query' => array(
                ...
                array(
                    'key' => self::META_COD_COLLECTED_AMOUNT,
                    'value' => '0',
                    'compare' => '>',
                    'type' => 'NUMERIC',
                ),
                array(
                    'key'     => '_lexi_courier_settled',
                    'compare' => 'NOT EXISTS',
                ),
            ),
        ));

        foreach ($collected_orders as $oid) {
            $amt = get_post_meta($oid, self::META_COD_COLLECTED_AMOUNT, true);
            if (is_numeric($amt)) {
                $total_collected_today += (float) $amt;
            }
        }
        ...
        return Lexi_Security::success(array(
            ...
            'total_collected_today' => round($total_collected_today, 2),
            'is_available' => self::is_available($user_id),
        ));
    }
    ...
    public static function update_order_status(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ('out-for-delivery' === $status || 'out_for_delivery' === $status) {
            ...
            $customer_event_title = 'الطلب في الطريق إليك';
            $customer_event_body = sprintf(
                'طلبك رقم #%s مع المندوب وفي الطريق إليك الآن.',
            ...
            $customer_event_title = 'تم تسليم الطلب';
            $customer_event_body = sprintf(
                'تم تسليم طلبك رقم #%s بنجاح. شكراً لتسوقك معنا.',
                (string) $order->get_order_number()
            );
    ...
    public static function collect_cod(WP_REST_Request $request): WP_REST_Response
    {
        ...
        self::notify_customer_status_update(
            ...
            'order_delivered',
            'تم تسليم الطلب',
            sprintf('تم تسليم طلبك رقم #%s وتأكيد دفع المبلغ.', (string) $order->get_order_number())
        );
