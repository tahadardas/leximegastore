...
class Lexi_Routes_Checkout
{
    ...
    private const ATTACH_TOKEN_TTL_SECONDS = 15 * MINUTE_IN_SECONDS;
    ...
    private static function create_order_from_payload(array $body): WP_REST_Response
    {
        ...
        self::logger()->info(
            sprintf(
                '[CHECKOUT] create-order request method=%s user_id=%d city_id=%d items=%d',
                $method,
        ...
        try {
            ...
            if (Lexi_Order_Flow::PAYMENT_METHOD_SHAMCASH === $method) {
                $order->update_meta_data('_lexi_decision', 'pending');
            }

            $order->calculate_totals();
            $order->save();
            self::log_order_state($order, 'after_initial_save');

            if (class_exists('Lexi_Order_Events')) {
                Lexi_Order_Events::log(
                    (int) $order->get_id(),
                    'order_created',
                    $customer_id > 0 ? 'customer' : 'system',
            ...
            if ('cod' === $method) {
                ...
                $order->update_status(
                    ...
                    'طلب شام كاش بانتظار رفع إيصال الدفع.'
                );
                if (class_exists('Lexi_Emails')) {
                    Lexi_Emails::send_internal_order_email_once($order, 'checkout_shamcash_created');
            ...
            $attach_token_data = self::issue_attach_token($order);
            $order->save();
            self::log_order_state($order, 'before_response');
            $attach_payload = self::build_attach_payload($order, $attach_token_data);
            $next_action['attach_device'] = $attach_payload;
    ...
    {
        $value = Lexi_Order_Flow::normalize_payment_method($raw);
        if (in_array($value, array('cod', Lexi_Order_Flow::PAYMENT_METHOD_SHAMCASH), true)) {
            return $value;
        }
    ...
    /**
     * @return array{token:string,expires_at:int,expires_at_iso:string}
     */
    private static function issue_attach_token(WC_Order $order): array
    {
        $token = wp_generate_password(40, false, false);
        $expires_at = time() + self::ATTACH_TOKEN_TTL_SECONDS;
        $token_hash = hash_hmac('sha256', $token, wp_salt('auth'));

        $order->update_meta_data('_lexi_attach_token_hash', $token_hash);
        $order->update_meta_data('_lexi_attach_token_expires', (string) $expires_at);
        $order->update_meta_data('_lexi_attach_token_issued_at', (string) time());

        return array(
            'token' => $token,
            'expires_at' => $expires_at,
            'expires_at_iso' => gmdate('c', $expires_at),
        );
    }

    /**
     * @param array{token:string,expires_at:int,expires_at_iso:string} $token_data
     * @return array<string,mixed>
     */
    private static function build_attach_payload(WC_Order $order, array $token_data): array
    {
        $endpoint = '/wp-json/' . LEXI_API_NAMESPACE . '/orders/' . (int) $order->get_id() . '/attach-device';

        return array(
            'endpoint' => $endpoint,
            'attach_token' => (string) ($token_data['token'] ?? ''),
            'expires_at' => (string) ($token_data['expires_at_iso'] ?? ''),
            'expires_in' => max(0, absint($token_data['expires_at'] ?? 0) - time()),
        );
    }
    ...
    private static function log_order_state(WC_Order $order, string $stage): void
    {
        ...
        self::logger()->info(
            sprintf(
                '[CHECKOUT] %s order_id=%d order_number=%s wc_status=%s post_status=%s post_type=%s payment_method=%s lexi_payment_method=%s',
                $stage,
                $order_id,
