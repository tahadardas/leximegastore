...
class Lexi_Routes_Admin
{
    ...
    public static function register(): void
    {
        ...
        register_rest_route($ns, '/admin/couriers/(?P<id>\d+)/settle', array(
            'methods' => WP_REST_Server::EDITABLE,
            'callback' => array(__CLASS__, 'settle_courier_account'),
            'permission_callback' => array('Lexi_Security', 'admin_access'),
            'args' => array(
                'id' => array('required' => true, 'sanitize_callback' => 'absint'),
            ),
    ...
    /**
     * PATCH /admin/couriers/{id}/settle
     */
    public static function settle_courier_account(WP_REST_Request $request): WP_REST_Response
    {
        $courier_id = (int) $request->get_param('id');
        $courier = get_user_by('id', $courier_id);
        if (!($courier instanceof WP_User) || !self::user_is_delivery_agent($courier)) {
        ...
        foreach ($orders as $order) {
            if ($order instanceof WC_Order) {
                $collected = (float) $order->get_meta('_lexi_cod_collected_amount');
                if ($collected > 0) {
                    $total_settled += $collected;
                    $order->update_meta_data('_lexi_cod_settled', 'yes');
                    $order->update_meta_data('_lexi_cod_settled_at', gmdate('c'));
                    $order->update_meta_data('_lexi_cod_settled_by', (int) get_current_user_id());
                    $order->add_order_note(sprintf('تم تسديد مبلغ التحصيل (%.2f) من المندوب إلى الإدارة.', $collected));
                    $order->save();
        ...
        return Lexi_Security::success(array(
            'message' => 'تم تسديد الحساب بنجاح.',
            'settled_amount' => round($total_settled, 2),
            'orders_affected' => count($order_ids),
        ));
    }
    ...
    private static function list_courier_event_rows(
        ...
        foreach ($rows as $row) {
            ...
            $amt = isset($row['amount']) && is_numeric($row['amount']) ? (float) $row['amount'] : null;
            if ($amt === null && $event_type === 'cod_collected' && isset($payload['received']) && is_numeric($payload['received'])) {
                $amt = (float) $payload['received'];
            }
