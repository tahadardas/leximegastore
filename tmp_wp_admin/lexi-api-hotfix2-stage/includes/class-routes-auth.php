...
class Lexi_Routes_Auth
{
    ...
    public static function register(): void
    {
        ...
        // Password reset flow (public - no JWT required).
    ...
    public static function login_customer(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $raw_username = trim((string) ($body['username'] ?? ''));
        $email = sanitize_email((string) ($body['email'] ?? ''));
        $identifier = '';

        // 1) Explicit email field has top priority.
        ...
        // 2) Backward-compatible: many clients send email in "username".
        if ('' === $identifier && '' !== $raw_username) {
            $candidate_email = sanitize_email($raw_username);
            if (false !== strpos($raw_username, '@') && is_email($candidate_email)) {
                $user_by_email = get_user_by('email', $candidate_email);
                if ($user_by_email instanceof WP_User) {
                    $identifier = (string) $user_by_email->user_login;
                }
            }
        ...
        // 3) Try exact login as entered before strict sanitization.
        if ('' === $identifier && '' !== $raw_username) {
            $user_by_login = get_user_by('login', $raw_username);
            if ($user_by_login instanceof WP_User) {
                $identifier = (string) $user_by_login->user_login;
            }
        }

        // 4) Login by phone number (billing_phone / phone user meta).
        if ('' === $identifier && '' !== $raw_username) {
            $identifier = self::find_login_by_phone($raw_username);
        }

        // 5) Last resort: sanitized username.
        if ('' === $identifier && '' !== $raw_username) {
            $identifier = sanitize_user($raw_username, true);
        }

        if ('' === $identifier || '' === $password) {
            return Lexi_Security::error('invalid_credentials', 'البريد الإلكتروني/اسم المستخدم وكلمة المرور مطلوبة.', 422);
        }

        $user = wp_authenticate($identifier, $password);
        if (is_wp_error($user) || !($user instanceof WP_User)) {
            return Lexi_Security::error('invalid_credentials', 'بيانات تسجيل الدخول غير صحيحة.', 401);
        }
    ...
    public static function register_customer(WP_REST_Request $request): WP_REST_Response
    {
        ...
        // 1) Phone is now mandatory as it's our primary anchor.
        if ('' === $phone || strlen($phone) < 9) {
            return Lexi_Security::error('invalid_phone', 'رقم الهاتف مطلوب ويجب أن يكون صحيحاً.', 422);
        }

        // 2) Email: use provided or generate from phone.
        $email = sanitize_email((string) ($body['email'] ?? ''));
        if ('' === $email) {
            $email = $phone . '@leximega.store';
        }
        ...
        // 3) Username: use provided or generate from phone.
    ...
    /**
     * Resolve user_login from phone number stored in user meta.
     */
    private static function find_login_by_phone(string $raw_phone): string
    {
        global $wpdb;
        ...
        $candidates = self::phone_login_candidates($raw_phone);
        if (empty($candidates)) {
            return '';
        }

        $meta_keys = array('billing_phone', 'phone');
        $meta_placeholders = implode(', ', array_fill(0, count($meta_keys), '%s'));
        $value_placeholders = implode(', ', array_fill(0, count($candidates), '%s'));

        $sql = "
            SELECT um.user_id
            FROM {$wpdb->usermeta} um
            WHERE um.meta_key IN ({$meta_placeholders})
              AND um.meta_value IN ({$value_placeholders})
            ORDER BY um.umeta_id DESC
            LIMIT 1
        ";

        $prepared = $wpdb->prepare($sql, array_merge($meta_keys, $candidates));
        if (!is_string($prepared) || '' === trim($prepared)) {
            return '';
        }

        $user_id = (int) $wpdb->get_var($prepared);
        if ($user_id <= 0) {
            return '';
        }
    ...
    /**
     * Build possible stored phone variants for login lookup.
     *
     * @return array<int, string>
     */
    private static function phone_login_candidates(string $raw_phone): array
    {
        $normalized = Lexi_Security::sanitize_phone($raw_phone);
        if ('' === trim($normalized)) {
            return array();
        }

        $digits = preg_replace('/\D+/', '', $normalized);
        $without_zero = ltrim((string) $digits, '0');
        ...
        if ('' !== $without_zero) {
            $candidates[] = $without_zero;
            $candidates[] = '0' . $without_zero;
            $candidates[] = '963' . $without_zero;
            $candidates[] = '+963' . $without_zero;
            $candidates[] = '00963' . $without_zero;
        }

        $clean = array_values(array_unique(array_filter($candidates, static function ($value): bool {
    ...
    public static function forgot_password(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (class_exists('Lexi_Text')) {
            ...
            $message = Lexi_Text::normalize($message);
        }
