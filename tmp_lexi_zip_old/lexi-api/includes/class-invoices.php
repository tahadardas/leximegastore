...
class Lexi_Invoices
{
    ...
    public static function render_html(int $order_id, string $type): string
    {
        ...
        $logo_html = '';
        if ('' !== $logo_url) {
            $logo_html = "<img src='" . esc_url($logo_url) . "' alt='Lexi Logo' class='brand-logo' />";
        }
...
<head>
	...
	<style>
		* { margin: 0; padding: 0; box-sizing: border-box; }
		...
		.header { background: linear-gradient(135deg, #111827 0%, #1f2937 60%, #374151 100%); color: #fff; padding: 26px 32px 30px; display: flex; justify-content: space-between; align-items: center; gap: 18px; border-bottom: 4px solid #facc15; }
		.header h1 { font-size: 24px; line-height: 1.15; }
		.brand { display: flex; align-items: center; gap: 12px; min-width: 0; }
		.brand-copy { min-width: 0; }
		.brand-logo { width: 52px; height: 52px; border-radius: 12px; background: rgba(255,255,255,.95); object-fit: contain; padding: 5px; box-shadow: inset 0 0 0 1px rgba(255,255,255,.4); }
		...
		<div class='header'>
			<div class='brand'>
				...
				<div class='brand-copy'>
					...
					<p style='margin-top:4px;opacity:0.88;font-size:14px;'>{$title}</p>
				</div>
			</div>
