...
class Lexi_Routes_Orders
{
    ...
    public static function render_invoice_qr(WP_REST_Request $request): WP_REST_Response
    {
        ...
        // Minimal QR via Google Chart is not allowed (external). We fallback to a simple placeholder
        // if GD is not available.
        if (!function_exists('imagecreatetruecolor')) {
            $html = '<svg xmlns="http://www.w3.org/2000/svg" width="' . (int) $size . '" height="' . (int) $size . '"><rect width="100%" height="100%" fill="#fff"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="14" fill="#111">QR</text></svg>';
        ...
        // NOTE: For production-grade QR, we recommend adding a small embedded QR lib.
        // For now, we render a deterministic visual marker based on hash to avoid external deps.
