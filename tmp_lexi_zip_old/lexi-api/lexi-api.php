...
define('LEXI_API_DB_VERSION', '1.3.6');
...
add_action('plugins_loaded', function () {
	...
	if (isset($_SERVER['REQUEST_METHOD']) && 'OPTIONS' === $_SERVER['REQUEST_METHOD']) {
		// Restrict interception to Lexi REST namespace only.
 ...
 *   %2525D8 -> %25D8 -> %D8
 *
 * This keeps encoded path bytes intact while preventing broken proxy URLs.
 */
function lexi_api_decode_over_encoded_url(string $url): string
{
	...
	for ($i = 0; $i < 8; $i++) {
		$next = preg_replace_callback('/%25([0-9a-fA-F]{2})/', function ($matches) {
			$byte = isset($matches[1]) ? strtoupper((string) $matches[1]) : '';
			return '' === $byte ? '%25' : ('%' . $byte);
		}, $current);
...
/**
 * Normalize media proxy URL while preserving already encoded UTF-8 bytes.
 *
 * esc_url_raw() may re-encode percent bytes from Arabic filenames; we decode
 * over-encoding again after sanitization to keep canonical upload URLs.
 */
function lexi_api_normalize_media_proxy_url(string $raw_url): string
{
	...
	if (false !== strpos($value, '%25')) {
		$value = lexi_api_decode_over_encoded_url($value);
	}
	...
	if (false !== strpos($sanitized, '%25')) {
		$sanitized = lexi_api_decode_over_encoded_url($sanitized);
	}
